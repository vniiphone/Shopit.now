...
public class UserAccountServices implements MandatoryUserServices{
    ...
    public List<UserView> allUsers() {
        ...
        for(User user:all){
            String fullname=registerUserRepository.getusername(user.getId());
    ...
    public ResponseEntity<String> registerTheUser(Register register) throws UserAlreadyExists {
        Register checkUser=registerUserRepository.findByEmail(register.getEmail().toLowerCase());
        if(checkUser!=null) {
    ...
    public ResponseEntity<String> addUserAddress(int id, Address address) throws UserNotFound {
        Register register=registerUserRepository.findById(id).orElse(null);
        if(register!=null){
    ...
    @Override
    public ResponseEntity<String> changePassword(int id, VerifyPassword verifyPassword) throws InvalidCredentials, UserNotFound {
        Register register = registerUserRepository.findById(id).orElse(null);
        if (register == null) {
            throw new UserNotFound("User not found");
        }
        else if(!bCryptPasswordEncoder.matches(verifyPassword.getPassword(),register.getPassword())){
    ...
    @Override
    public ResponseEntity<String> deleteAccount(int id, ConfirmPassword confirmPassword) throws UserNotFound, InvalidCredentials {
        Register register = registerUserRepository.findById(id).orElse(null);
        if (register == null) throw new UserNotFound("User not found");
        if(!bCryptPasswordEncoder.matches(confirmPassword.getPassword(),register.getPassword())) throw new InvalidCredentials("User password wrong");
        try{
    ...
    public ResponseEntity<String> updateAddress(int userId, Address address) throws UserNotFound, AddressNotFound {
        Register register = registerUserRepository.findById(userId).orElse(null);
        if (register == null) {
            throw new UserNotFound("User not found");
        }
        else {
            List<Address> found = register.getUser().getAddress();
            if(found.size()==0){
    ...
    public ResponseEntity<String> updateDefaultAddress(int userId, int aId) throws UserNotFound{
        ...
        try {
            ...
            for(Address address:addresses){
                if(address.getId()==aId) address.setDefaultAddress(true);
                else address.setDefaultAddress(false);
            }
    ...
    public List<CartDetails> cart(int userId) throws UserNotFound, ProductNotFound, GlobalServerException {
        ...
        for (CartDetails c:cartDetails){
            ...
            if(instock-cartStock<0) c.setAvailable(false);
    ...
    public ResponseEntity<Boolean> incrementCart(int userId, int productId,int itemCount) throws UserNotFound {
       ...
       if(available<=0) availableUpdate=false;
       else availableUpdate=true;
    ...
    public ResponseEntity<Boolean> decrementCart(int userId, int productId,int itemCount) throws UserNotFound {
        ...
        boolean availableUpdate=true;
        if(available<=stock) availableUpdate=true;
        else availableUpdate=false;
        return cartManipulation(userId,productId,-1,availableUpdate);
    }

    private ResponseEntity<Boolean> cartManipulation(int userId,int productId,int num,boolean availableUpdate) throws UserNotFound {
        User user =userRepository.findById(userId).orElse(null);
        if(user==null) throw new UserNotFound("User Not Found");
    ...
    public ResponseEntity<String> removeFromCart(int userId, int productId) throws UserNotFound {
        ...
        User user =userRepository.findById(userId).orElse(null);
        if(user==null) throw new UserNotFound("User Not Found");
        List<CartDetails> newCart=user.getCart()
                .stream()
    ...
    public ResponseEntity<String> removeFromSaveLater(int userId, int productId) throws UserNotFound {
        User user =userRepository.findById(userId).orElse(null);
        if(user==null) throw new UserNotFound("User Not Found");
        List<SaveForLater> newCart=user.getSaveForLater()
                .stream()
                .filter(i->i.getProductId()!=productId)
                .collect(Collectors.toList());
    ...
    public ResponseEntity<String> addToSaveForLater(int userId, SaveForLater saveForLater) throws UserNotFound {
        ...
        List<CartDetails> newCartDetails=oldCartDetails
                .stream()
                .filter(i->i.getProductId()!=saveForLater.getProductId())
                .collect(Collectors.toList());
    ...
    public ResponseEntity<String> addBackToCart(int userId, CartDetails cartDetails) throws UserNotFound {
        ...
        List<SaveForLater> newSaveForLater=oldSaveForLater
                .stream()
                .filter(i->i.getProductId()!=cartDetails.getProductId())
                .collect(Collectors.toList());
    ...
    public ResponseEntity<String> addOrder(int userId,List<Orders> orders) throws UserNotFound, ParseException {
        ...
        for(Orders o:orders){
            ...
            if(newStock<0) return new ResponseEntity<>("Unknown Error occured",HttpStatus.BAD_REQUEST);
    ...
    public List<WishlistView> getWishlist(int userId) throws UserNotFound {
        ...
        for(Wishlist w:wishlist){
            ...
            if(product!=null){
                ...
                WishlistView wishlistView=new WishlistView(w.getId(),product.getId(),product.getTitle(), product.getPrice(),product.getThumbnail(),available);
    ...
    public boolean handleWishlist(int userId, int productId) throws UserNotFound, GlobalServerException {
        try{
            ...
            if(wishlist==null) {
                User user=userRepository.findById(userId).orElse(null);
                if(user!=null) wishListRepository.save(new Wishlist(productId,user));
                return true;
            }else{
    ...
    @Override
    @Transactional
    public ResponseEntity<String> wishlistToCart(int userId, int productId,int wishlistId) throws UserNotFound, GlobalServerException {
        boolean check= checkWishList(userId,productId);
        if(check==false)
            return new ResponseEntity<>("Wishlist already removed",HttpStatus.BAD_REQUEST);
        Products products=productRepository.findById(productId).orElse(null);
        if(products==null) return new ResponseEntity<>("Product not found",HttpStatus.NOT_FOUND);
