...
import {
  removeFromSaveLater,
  fetchSaveLater,
  fetchSaveLaterSuccess,
  fetchSaveLaterError,
  moveToSaveLater,
} from "./SaveForLaterActions";
import {
  saveLaterSubmitToServer,
  saveLaterSubmitError,
  saveLaterSubmitSuccess,
} from "./SaveLaterServerSubmitActions";
import {
  submitToServer,
  serverError,
  serverSuccess,
} from "../Cart/ServerSubmitActions";
...
const saveForLaterReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case MOVE_TO_CART:
      return {
        ...state,
        items: [...removeFromSaveLaterUI(state.items, action.payload)],
      };
...
const fetchSaveForLater = () => {
  ...
  return (dispatch) => {
    ...
    axios
      .get(`${URL(API_ENDPOINT.userSaveLater)}/${id}`, {
        headers: {
...
const removeFromSaveLaterServer = (productId) => {
  ...
  return (dispatch) => {
    ...
    axios
      .delete(`${URL(API_ENDPOINT.userSaveLater)}/remove/${id}/${productId}`, {
        headers: {
          Authorization: getJwt(),
        },
      })
...
const addToSaveLaterServer = (item) => {
  ...
  return (dispatch) => {
    ...
    axios
      .post(
        `${URL(API_ENDPOINT.userSaveLater)}/add/${id}`,
        { productId, productName, totalPrice, itemCount },
...
const addBackToCart = (item) => {
  ...
  return (dispatch) => {
    ...
    axios
      .post(
        `${URL(API_ENDPOINT.userCart)}/back-to-cart/${id}`,
        { productId, productName, totalPrice, itemCount },
        { headers: { Authorization: getJwt() } }
      )
      .then(() => {
