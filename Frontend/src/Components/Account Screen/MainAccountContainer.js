...
import {getCurrentUser,getJwt,removeJwt,userLogin,setJwt} from '../../services/LoginReg';

class MainAccountContainer extends Component {
    data=[{name:"Orders",to:"/account/orders"},{name:"Manage Address",to:"/account/address"}]
    ...
    getImageFromServer=async(id)=>{
        await axios.get(`${api.userOperations}/profile-image/${id}`,{
            ...
            if(data){const imgUrl=`data:${data.type};base64,${data.picByte}`;
    ...
    handleAccountDelete=async()=>{
        ...
        if(confirmPassword.length>0){
            ...
            await axios.post(`${api.userOperations}/delete-user/${user.id}`, {password:confirmPassword},
                {headers:{'Authorization':getJwt()}})
                .then(()=>{
                    removeJwt();
                }).catch(({response})=>{
                    if(response.status===401){
                        this.setState({error:"Password is wrong",submissionStarted:false});
                    }else{
                        this.setState({error:"Server error. Try after sometime",submissionStarted:false});
                    }
                })
        }
        else{
    ...
    handlePasswordChange=async(value)=>{
        ...
        await axios.put(`${api.userOperations}/update-password/${user.id}`,{
    ...
    uploadImageDataToServer=async(uploadData,image)=>{
        ...
        await axios.post(`${api.userOperations}/profile-image/${user.id}`,uploadData,{
            headers:{'Authorization':getJwt()}
        }).catch(()=>{
            this.setState({imageError:"Sorry cannot upload the image right now. Try agian later",profileImage:oldImage});
        })
    }
