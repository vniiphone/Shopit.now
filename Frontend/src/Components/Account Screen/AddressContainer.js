...
class AddressContainer extends Component {
    ...
    async componentDidMount(){
        ...
        await axios.get(`${api.userOperations}/address/${user.id}`,{
            headers:{
    ...
    changeDefaultInServer=async(account,item)=>{
        ...
        await axios.put(`${api.userOperations}/update-default-address/${user.id}/${item}`,null,{
    ...
    handleEdit=(item)=>{
     ...
     editChangesToServer=async(address)=>{
        ...
        await axios.put(`${api.userOperations}/update-address/${user.id}`,address,{
            headers:{
                'Authorization':getJwt()
            }
        }).then(()=>{
    ...
    deleteAddress=async(item)=>{
        const {user}=this.state;
        this.removeError();
        await axios.delete(`${api.userOperations}/delete-address/${user.id}/${item}`,
            {headers:{'Authorization':getJwt()}
            }).then(()=>{this.updateAddressAfterDeletion(item);})
    ...
    addAddress=(values,onSubmitProps)=>{
     ...
     addAddressToServer=async(address)=>{
        const {user} =this.state;
        this.setState({onSubmission:true});
        await axios.put(`${api.userOperations}/address/${user.id}`,address,{
            headers:{'Authorization':getJwt()}
        }).then(()=>{
     ...
     errorhandle=()=>{
        ...
        this.setState({submissionError:"Server error occured. Try after sometime"});
     }
     removeError=()=>{
