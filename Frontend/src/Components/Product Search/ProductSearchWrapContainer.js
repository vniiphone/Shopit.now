...
import { StarRate,TodaysDeal,Available,scroll,defaultPrice,defaultRating,defaultDeal,defaultStock,defaultPage,defaultPageSize } from "./Utils";
...
export default class ProductSearchWrapContainer extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({query});
        this.getProducts(query,defaultPageSize,defaultPage,defaultPrice,defaultRating,defaultStock,defaultDeal,1);
    }
    ...
    resetFilter=(query,dataFound)=>{
        this.setState({filterVisible:true,query,currentPage:defaultPage,priceRange:defaultPrice,rating:defaultRating,stock:defaultStock,deal:defaultDeal,filterDisp:[],start:1,end:3});
        this.getProducts(query,defaultPageSize,defaultPage,defaultPrice,defaultRating,defaultStock,defaultDeal,dataFound);
    }

    getProducts = async (query,pagesize,currentPage,priceRange,rating,outOfStock,topDeals,dataFound) => {
        this.setState({ error: false,loading:true});
        await axios.get(`${api.productApi}/search/${query}`,{
            params:{
                limit:pagesize,
                page:currentPage-1,
                price:priceRange,
            ...
            if(searchView.length<1){
                dataFound===2? this.setState({filterVisible:true}) : this.setState({filterVisible:false});
            }
    ...
    handlePriceDebounce=debounce((val)=>
        this.priceServerConnect(val),500
    )
    ...
    filterDisplayHandle=(option,what)=>{
        ...
        let newstock=stock;
        let newdeal=deal;
        (option===Available ? (what? newstock=true : newstock=false):(option===TodaysDeal && (what? newdeal=true : newdeal=false)));
        this.getProducts(query,pagesize,1,priceRange,rating,newstock,newdeal,2);
    ...
    ratingUpdation=(star,what)=>{
        ...
        const updated= what ? filter=[...filter,`${StarRate} - ${star}+`] : filter ;
        const updatedRating= what ? star : defaultRating;
        this.setState({filterDisp:updated,rating:updatedRating,currentPage:1,start:1,end:3});
        this.getProducts(query,pagesize,1,priceRange,updatedRating,stock,deal,2);
    }
    ...
    incrementPage=(size)=>{
        const {start,end,currentPage,pagesize,query,priceRange,rating,stock,deal}=this.state;
        this.handleScroll();
        if(currentPage===end-1 && currentPage!==size-2) this.setState({start:start+1,end:end+1});
        this.setState({currentPage:currentPage+1});
        this.getProducts(query,pagesize,currentPage+1,priceRange,rating,stock,deal,2);
    }

    decrementPage=()=>{
        const {start,end,pagesize,currentPage,query,priceRange,rating,stock,deal}=this.state;
        this.handleScroll();
