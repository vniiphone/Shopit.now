...
class Notifications extends Component {
    ...
    getNotifications=async()=>{
        await axios.get(`${api.userOrders}/notification`,{
            headers:{
    ...
    storeAllNotification=(data)=>{
        ...
        this.setState({notifications:updatedData,notificationCount:data.length,notificationError:false});
    }

    viewNotifications=async(count)=>{
        await axios.put(`${api.userOrders}/notification/seen`,null,{
            headers:{
                "Authorization":getJwt()
            }
        }).then(()=>{
    ...
    render() {
        const {notificationToggle,notifications,notificationError,notificationCount} =this.state;
        return (
            <div className="notification-container">
                {notificationCount>0 && <h3>{notificationCount}</h3>}
                <FontAwesomeIcon onClick={this.handleNotification} icon={faBell} />
                <div className={`notifications-display ${notificationToggle?"notifications-display-visible":""}`}>
                   {notificationError ?
                    ...
                    notifications.length===0?
                        ...
                        notifications.map((i,index)=>(
                            <div onClick={this.refreshOrders} key={index} className="notification-display-sub">
                                <h2>New Order Placed..</h2>
                                <h5>Order id: <span>{i.id}</span></h5>
                                <h5>Order: <span>{i.productName}</span></h5>
                            </div>
                    ))} 
