...
class DeleteAccount extends Component {
    ...
    performDeletion=async()=>{
        ...
        await axios.post(`${api.userOperations}/delete-user/${id}`, {password:deletionPass},
            ...
            .then(()=>{
                ...
                if(request.status===401){
                    ...
                    this.setState({error:"Server error. Try after sometime",loading:false});
                }
            })
        }else{
    ...
    render() {
        ...
        return (
            <div className="admin-settings-delete-container">
                ...
                {loading && <div className="admin-settings-delete-loading">
                    <Lottie className="loading-anim" animationData={loadingAnim} />
                </div>}
                <h4>Are you sure you want to delete your account? Once you delete your account, there is no way getting it back.</h4>
                <InputContainer disabled={loading?true:false} value={deletionPass}  onChange={this.storePassword} name="Confirm Password" />
                {error && <h5>{error}</h5>}
                <p><button disabled={loading?true:false} onClick={this.performDeletion}>Delete</button></p>
            </div>
        );
    }
}
