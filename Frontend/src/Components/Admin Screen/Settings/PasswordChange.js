    ...
    await axios
      ...
      .catch(({ request }) => {
        if (!request.status) {
          this.setState({
            passwordChangeLoading: false,
            passwordChangeServerError: true,
          });
        } else if (request.status === 401) {
          this.setState({
            passwordChangeLoading: false,
            passwordChangeError: "Password is wrong",
          });
        } else {
          this.setState({
            passwordChangeLoading: false,
  ...
  render() {
    ...
    return (
      <div className="admin-settings-password-container">
        ...
        {passwordChangeLoading && (
          <div className="admin-settings-pass-change-loading">
        ...
        >
          {({
            ...
            <form onSubmit={handleSubmit}>
              ...
              />
              {passwordChangeServerError && (
                <h5 className="admin-change-server-error">
                  Server error occured. Try after sometime
                </h5>
              )}
              <p>
                <button
