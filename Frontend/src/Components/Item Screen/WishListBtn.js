...
class WishListBtn extends Component{
    ...
    getWishlisted=async()=>{
        ...
        if(getCurrentUser() && productId){
            ...
            await axios.get(`${api.userOperations}/wishlist/check/${id}/${productId}`,{
                headers:{
    ...
    handleWishListOperation=async(id)=>{
        ...
        if(productId){
            ...
            await axios.post(`${api.userOperations}/wishlist/${id}/${productId}`,null,{
                headers:{
                    'Authorization':getJwt()
                }
            }).then(({data})=>{
...
render(){
    ...
    return(
        <>
            <button disabled={loading?true:serverError?true:false} onClick={this.handleWishlist} className="wishlist-btn">
                {loading && <><Lottie animationData={loadingAnim} className="wishlist-btn-loading" />
                <div className="wishlist-btn-loading-c" /></>}
                <FontAwesomeIcon icon={wishlisted?heartFill:heartUnFilled} color={ wishlisted?"red":""} />
                <span>Wishlist{wishlisted && "ed" }</span>
                {serverError && <h4>Cannot use it right now, try again later.</h4>}
            </button>
            {error && <div className={`wishlist-error`}>
