...
class MainProductContainer extends Component {
    ...
    itemCheck=async()=>{
        ...
        await axios.get(`${api.productApi}/view/get-product/${item}`)
        .then(({data:found})=>{
    ...
    successfullSubmission=(found)=>{
        ...
        const loadedReview=found.productRatings.usersReviews.filter((i,index)=>index<3);
        const overallRating=Math.round((found.productRatings.overallRating)* 10)/10;
        this.setState({product:found,loading:false,loadedReview,overallRating,initialUrl});
    }

    checkSubmission=(user)=>{
        const {product}=this.state;
        const check=product.productRatings.usersReviews.find(i=>i.email===user);
        if(check)
            return true;
        return false;
    }
    ...
    handleReviewSubmit=()=>{
        ...
        if(this.checkSubmission(user.sub)){
            const submissionError=document.querySelector(".review-btn-container h5");
            ...
            const starRating=document.querySelector(".star-rating-container .star-rating-error");
            ...
            this.reviewToServer(newReview,product.id,product,newReview,loadedReview);
        }
    }

    reviewToServer=async(review,id,product,newReview,loadedReview)=>{
        this.setState({reviewSubmit:true}); 
        await axios.post(`${api.productApi}/add-product-review/${id}`,review,{
    ...
    refactorTheChanges=(product,newReview,loadedReview)=>{
        ...
        this.setState({product:updatedProduct,loadedReview:updatedLoadedReview,review:'',rating:0,overallRating:total});
    }
    ...
    handleLoadMore=()=>{
        ...
        const reviews=product.productRatings.usersReviews.filter((i,index)=>index<=size);
        const container=document.querySelector(".review-load-more");
        if(len<=size)
            container.classList.add("remove-review-load-more");
        else
            container.classList.remove("remove-review-load-more");
