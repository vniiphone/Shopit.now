...
class TodaysDeals extends Component {
    ...
    async componentDidMount(){
        ...
        await axios.get(`${api.productApi}/view/new-deals`).then(({data})=>{
        ...
        return (
            <div className="todays-deal-container">
                <div className="todays-deal-head">
                    <h2>Today's Deals</h2>
                    {loading && <Lottie className="new-deals-load" animationData={load} />}
                    {(!loading && !error && data.length!==0) &&<span><TimeGen /></span>}
                </div>
                {error ? 
                    <div className="todays-deal-error">
