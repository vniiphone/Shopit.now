...
class Cart extends Component {
    ...
    checkStock=(cart)=>{
        ...
        cart.forEach(async(i)=>{
            ...
            await axios.get(`${api.userCart}/check-stock/${productId}/${itemCount}`,{
                headers:{
            ...
            <>
                <div className="cart-container">
                    <div className="cart-left-container">
                        <h2>Shopping Cart <span>{len===1? `(${len} item)`: (len>1?`(${len} items)`:"" )}</span></h2>
                        {loading? <CartLoadin/>:
                            error?<CartError/>:
                                len<1 ? <CartEmpty/>: 
                                    <CartItems removeOutofStock={this.removeOutofStock} unavailable={unavailable}/>
                        }
                    </div>
                    { len>0 && <ProceedOrder proceedLoading={proceedLoading} unavailable={unavailable} proceedOrder={this.proceedOrder}/> }
                </div>
                <SaveForLater/>
            </>
        );
